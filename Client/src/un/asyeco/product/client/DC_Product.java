...
public class DC_Product extends ClientDocument implements C_Product {
	...
	public void define_ReferenceDataRule(D_Product doc, VD_Product vd ) {
		...
		doc.addRule(new R_New_Itm(), new KernelEvent(MEN_ITM_NEW));
		doc.addRule(new R_Delete_Itm(vd), new KernelEvent(MEN_ITM_DEL));
		// Enable and disable the new (MEN_ITM_NEW) and delete (MEN_ITM_DEL)
		doc.addRule(new R_ManageItmVisualEvents(), EventConstants.GUI_FORM_GAIN_FOCUS);
...
//		
		...
		doc.ds(CMP).ds(PROD).de(COD).addRule(ruleTarget6, DATA_VERIFY);
