...
public class R_EndOfTransaction extends Rule implements C_Product {

	// This rules displays the end of transaction dialog, and execute user's
	// selection.

	private D_Product doc;

	private VD_Product vd;

	public R_EndOfTransaction(VD_Product vd) {
		this.vd = vd;
		this.doc = (D_Product) vd.getDocument();
	}
...
//		}
		...
		if (option == KEndOfTransactionDialog.FORWARD) {
			...
			String ope = OC_NEW;
			doc.fire(new StartOperationEvent(doc, "un.asyeco.Product.D_Product", ope));
		}
