...
public class VR_Done extends Rule implements C_eCO {

	D_eCO doc;

	VD_eCO vd;

	public VR_Done(D_eCO doc, VD_eCO vd) {
		this.doc = doc;
		this.vd = vd;
	}
	...
	public void apply(KernelEvent e) {
		...
		if ((op.getID() == OI_ACCEPT) ) { // GVA <patch ID="Integration of regional transit" version="4.2.2" type="modification" date="Nov 27, 2013" author="ahmed" />
			...
			message = lng(arg, new Object[] { serialNbrField.getContent(), crnField.getContent() });
		...
		// Print selections
		JCheckBox printDocument = new JCheckBox(lng("Print CO document, customised"));
		...
		if (option == KEndOfTransactionDialog.FORWARD) {
			...
			if ((op.getID() == OI_CREATE) ) {
				...
				doc.fire(new DoCloseEvent(ev.getDocument()));
				doc.fire(new StartOperationEvent(doc, "un.asyeco.eco.D_eCO", OC_NEW));
			}
		}
